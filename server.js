...
async function generateCartoonImage(prompt, outputPath, seed) {
  ...
  // ── Primary (FREE): Pollinations Flux — try original seed, then a retry seed ──
  ...
  const flux2 = `https://image.pollinations.ai/prompt/${fullPrompt}?width=768&height=1344&seed=${seed + 1}&nologo=true&model=flux&enhance=true`;
  // ── Secondary (FREE): Pollinations Turbo (different model/server) ──
  const turbo = `https://image.pollinations.ai/prompt/${fullPrompt}?width=768&height=1344&seed=${seed}&nologo=true&model=turbo`;

  for (const [label, url] of [['Pollinations Flux 🎨', flux1], ['Pollinations Flux (retry) 🎨', flux2], ['Pollinations Turbo 🎨', turbo]]) {
    try {
      ...
      console.log(`[Image] ${label} returned tiny response (${response.data ? response.data.length : 0} bytes, status ${response.status}), trying next...`);
    } catch (err) {
      const code = err.code || (err.response ? `HTTP ${err.response.status}` : 'unknown');
      console.log(`[Image] ${label} failed (${code}: ${err.message}), trying next...`);
      await new Promise(r => setTimeout(r, 1500));
    }
  }

  // ── Bonus (only if you've funded credits): ModelsLab text2img ──
  if (MODELSLAB_API_KEY) {
    try {
      ...
      const errData = err.response ? JSON.stringify(err.response.data).substring(0, 150) : err.message;
      console.log('[Image] ModelsLab skipped:', errData);
    }
  }
...
app.get('/status', (req, res) => res.json({
  ...
  imageSources: 'Pollinations Flux (free, primary) → Pollinations Turbo (free) → ModelsLab (bonus, if funded) → Picsum (last resort)',
  features: [
