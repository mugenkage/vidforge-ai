...
const ART_STYLES = [
  {
    name: 'Chibi Cartoon',
    prompt: 'chibi cartoon style, cute oversized head, tiny body, big sparkly eyes, bold black outlines, flat bright colors, kawaii, adorable expression'
  },
  {
    name: 'Anime Manga',
    prompt: 'anime manga style, big expressive eyes, dynamic hair, detailed shading, clean lines, vibrant colors, emotional expression, japanese animation'
  },
  {
    name: 'Comic Book Hero',
    prompt: 'american comic book style, bold ink outlines, dynamic pose, halftone shading, primary colors, superhero aesthetic, dramatic lighting'
  },
  {
    name: 'Pixar 3D',
    prompt: 'pixar 3d animation style, smooth surfaces, expressive cartoon face, colorful background, subsurface scattering skin, disney quality render'
  },
  {
    name: 'Retro Cartoon',
    prompt: 'retro 1950s cartoon style, rubberhose animation, simple round shapes, limited color palette, bold black outlines, vintage cartoon feel'
  },
  {
    name: 'Dark Anime',
    prompt: 'dark anime style, dramatic lighting, glowing eyes, detailed shading, cinematic, mature manga aesthetic, moody atmosphere, sharp lines'
  },
  {
    name: 'Storybook',
    prompt: 'childrens storybook illustration style, watercolor textures, soft pastel colors, whimsical characters, gentle lines, fairy tale aesthetic'
  },
  {
    name: 'Street Art Cartoon',
    prompt: 'graffiti street art cartoon style, bold flat colors, sharp geometric shapes, urban aesthetic, spray paint texture, expressive character'
  }
];
...
Return ONLY this exact JSON, no markdown:
{
  ...
  // Build full image prompt per scene: art style + same character + scene-specific emotion
...
// ─────────────────────────────────────────────
// STEP 2 — GEMINI IMAGEN 3 + Pollinations fallback
// ─────────────────────────────────────────────
async function generateCartoonImage(prompt, outputPath, seed) {
  ...
  if (GEMINI_API_KEY) {
    try {
      ...
      if (predictions && predictions[0] && predictions[0].bytesBase64Encoded) {
        fs.writeFileSync(outputPath, Buffer.from(predictions[0].bytesBase64Encoded, 'base64'));
        console.log('[Image] Gemini Imagen 3 ✅');
        return;
      }
    } catch (err) {
      const msg = err.response ? JSON.stringify(err.response.data).substring(0, 150) : err.message;
      console.log(`[Image] Gemini failed: ${msg} — trying Pollinations...`);
    }
  }
  ...
  const urls = [
    ...
    `https://image.pollinations.ai/prompt/${enc}?width=768&height=1344&seed=${seed + 1}&nologo=true&model=flux&enhance=true`,
    `https://image.pollinations.ai/prompt/${enc}?width=768&height=1344&seed=${seed}&nologo=true&model=turbo`
  ];
...
async function runPostingCycle(category = null) {
  ...
  console.log(`\n🎬 [VidForge v8] Category: ${selectedCategory}`);
...
app.get('/status', (req, res) => res.json({
  ...
  imageSource: GEMINI_API_KEY ? 'Gemini Imagen 3 (primary) + Pollinations (fallback)' : 'Pollinations only (add GEMINI_API_KEY)',
...
app.listen(PORT, () => {
  console.log(`\n🚀 VidForge AI v8.0 — Gemini Imagen 3 + ${ART_STYLES.length} Art Styles + ${CAPTION_STYLES.length} Caption Styles`);
  console.log(`🎨 Gemini: ${GEMINI_API_KEY ? 'ENABLED ✅' : 'NOT SET — using Pollinations fallback'}`);
