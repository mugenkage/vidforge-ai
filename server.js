...
// Every 6 hours = 4 videos/day (00:00, 06:00, 12:00, 18:00)
cron.schedule('0 */6 * * *', () => {
...
async function generateCartoonImage(prompt, outputPath, seed) {
  // ── Primary: Gemini Imagen 3 (free tier credits) ──────
  // ⚠️ Google is shutting down all Imagen models on June 24, 2026.
  // Kept as primary while it lasts; ModelsLab is the fallback after that.
  if (GEMINI_API_KEY) {
    try {
      const response = await axios.post(
        'https://generativelanguage.googleapis.com/v1beta/models/imagen-3.0-generate-002:predict',
        {
          instances: [{ prompt: `${prompt}, high quality, sharp focus` }],
      ...
      console.log('[Image] Imagen 3 returned no image:', JSON.stringify(response.data).substring(0, 200));
    } catch (err) {
      const errData = err.response ? JSON.stringify(err.response.data).substring(0, 200) : err.message;
      console.log('[Image] Imagen 3 failed:', errData, '- falling back to ModelsLab');
    }
  }

  // ── Fallback: ModelsLab text2img (Flux model) ─────────
  if (MODELSLAB_API_KEY) {
    try {
      ...
      console.log('[Image] ModelsLab returned no image:', JSON.stringify(response.data).substring(0, 200));
    } catch (err) {
      const errData = err.response ? JSON.stringify(err.response.data).substring(0, 200) : err.message;
      console.log('[Image] ModelsLab failed:', errData, '- falling back');
    }
  }

  const fullPrompt = encodeURIComponent(prompt);

  // ── Fallback (free, may require account/credits depending on Pollinations policy) ──
  const flux1 = `https://image.pollinations.ai/prompt/${fullPrompt}?width=768&height=1344&seed=${seed}&nologo=true&model=flux&enhance=true`;
  const turbo = `https://image.pollinations.ai/prompt/${fullPrompt}?width=768&height=1344&seed=${seed}&nologo=true&model=turbo`;

  for (const [label, url] of [['Pollinations Flux 🎨', flux1], ['Pollinations Turbo 🎨', turbo]]) {
    try {
      ...
      console.log(`[Image] ${label} returned tiny response (${response.data ? response.data.length : 0} bytes), trying next...`);
    } catch (err) {
      const code = err.code || (err.response ? `HTTP ${err.response.status}` : 'unknown');
      console.log(`[Image] ${label} failed (${code}), trying next...`);
      await new Promise(r => setTimeout(r, 1000));
    }
  }

  // ── Absolute last resort: random photo (keeps the pipeline alive) ──
  try {
    const url = `https://picsum.photos/seed/${seed}/768/1344`;
    const response = await axios({
      method: 'GET', url,
      responseType: 'arraybuffer',
      timeout: 60000,
...
async function resolveModelsLabImage(data) {
  ...
  if (data.status === 'processing' && data.id) {
    const fetchUrl = `https://modelslab.com/api/v6/images/fetch/${data.id}`;
    for (let i = 0; i < 10; i++) {
      await new Promise(r => setTimeout(r, 3000));
      try {
        const poll = await axios.post(fetchUrl, { key: MODELSLAB_API_KEY }, { timeout: 30000 });
...
app.get('/status', (req, res) => res.json({
  ...
  imageSources: 'Gemini Imagen 3 (primary, until Jun 24 2026 shutdown) → ModelsLab Flux (fallback) → Pollinations (bonus) → Picsum (last resort)',
  features: [
