...
// Shot angles — 3 variations per scene for cinematic feel
const SHOT_ANGLES = [
  'wide establishing shot, full body character visible, detailed background environment',
  'medium close-up shot, character from waist up, expressive face prominent, background slightly blurred',
  'dramatic close-up, character face filling frame, intense emotion, cinematic crop',
];
...
Return ONLY JSON, no markdown:
{
  ...
  // Build 3 image prompts per scene (different shot angles, same character)
  script.scenes = script.scenes.map((scene, i) => ({
    ...scene,
    imagePrompts: SHOT_ANGLES.map(angle =>
      `${artStyle.prompt}, ${script.character_design}, ${scene.emotion}, ${angle}, vertical 9:16 portrait, no text, no watermark, vibrant, high quality`
    )
  }));
...
// ─────────────────────────────────────────────
async function buildImageClip(imgPath, outputPath, duration, kenBurnsIndex) {
  const kenBurns = [
    `zoompan=z='min(zoom+0.001,1.06)':x='iw/2-(iw/zoom/2)':y='ih/2-(ih/zoom/2)':d=1:s=768x1344:fps=24`,
    `zoompan=z='1.05':x='iw*0.05+iw*0.05*on/300':y='ih/2-(ih/zoom/2)':d=1:s=768x1344:fps=24`,
    `zoompan=z='max(zoom-0.001,1.0)':x='iw/2-(iw/zoom/2)':y='ih/2-(ih/zoom/2)':d=1:s=768x1344:fps=24`,
    `zoompan=z='1.05':x='iw*0.1-iw*0.05*on/300':y='ih/2-(ih/zoom/2)':d=1:s=768x1344:fps=24`,
  ];
  const zoom = kenBurns[kenBurnsIndex % kenBurns.length];
  const vf = `scale=768:1344:force_original_aspect_ratio=decrease,pad=768:1344:(ow-iw)/2:(oh-ih)/2:black,${zoom}`;
  await runFFmpeg([
    '-loop', '1', '-i', imgPath,
...
// ─────────────────────────────────────────────
async function buildSceneVideo(imgPaths, caption, outputPath, duration, style, wordTimestamps, sceneIndex) {
  const fadeOut    = Math.max(duration - 0.3, 0);
  const clipDur    = duration / imgPaths.length; // split duration across 3 images

  // Build individual image clips
  const clipPaths = [];
  for (let i = 0; i < imgPaths.length; i++) {
    const clipPath = outputPath.replace('.mp4', `_clip${i}.mp4`);
    await buildImageClip(imgPaths[i], clipPath, clipDur, sceneIndex * 3 + i);
...
async function runPostingCycle() {
  ...
  try {
    ...
    const sceneImgPaths   = []; // array of [img0, img1, img2] per scene
    ...
    for (let i = 0; i < script.scenes.length; i++) {
      ...
      // Generate 3 images + voice in parallel
      const imgPaths = scene.imagePrompts.map((_, j) => path.join(tempDir, `img_${i}_${j}.png`));

      await Promise.all([
        ...scene.imagePrompts.map((prompt, j) =>
          generateCartoonImage(prompt, imgPaths[j], characterSeed) // SAME seed for character consistency
        ),
        generateVoice(scene.narration, audioPath, voice.id)
      ]);

      sceneImgPaths.push(imgPaths);
      ...
      console.log(`[VidForge] Scene ${i + 1} ready (${duration.toFixed(1)}s, 3 images)`);
    }
...
// ROUTES
// ─────────────────────────────────────────────
app.get('/', (req, res) => res.json({ app: 'VidForge AI', version: '12.2.0', status: 'running' }));
...
app.listen(PORT, () => {
  console.log(`\n🚀 VidForge AI v12.2 — Series Engine + 3 Images/Scene + End Card + Seamless Music`);
