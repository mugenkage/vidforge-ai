...
// FIXED: CF returns base64 JSON not arraybuffer
// FIXED: correct param is 'steps' not 'num_steps'
// Rate limit: 720 req/min — no delays needed
// ─────────────────────────────────────────────
const CF_MODELS = [
  { id: '@cf/black-forest-labs/flux-1-schnell',          body: (p, s) => ({ prompt: p, steps: 4, seed: s }) },
  { id: '@cf/stabilityai/stable-diffusion-xl-base-1.0', body: (p, s) => ({ prompt: p, seed: s }) },
  { id: '@cf/lykon/dreamshaper-8-lcm',                   body: (p, s) => ({ prompt: p, steps: 8, seed: s }) },
  { id: '@cf/bytedance/stable-diffusion-xl-lightning',   body: (p, s) => ({ prompt: p, steps: 4, seed: s }) },
];
let cfModelIndex = 0;

async function generateCartoonImage(prompt, outputPath, seed) {
  if (CF_ACCOUNT_ID && CF_API_TOKEN) {
    for (let attempt = 0; attempt < CF_MODELS.length; attempt++) {
      const model = CF_MODELS[cfModelIndex % CF_MODELS.length];
      cfModelIndex++;
      try {
        console.log(`[Image] Trying CF: ${model.id.split('/').pop()}`);
        const response = await axios.post(
          `https://api.cloudflare.com/client/v4/accounts/${CF_ACCOUNT_ID}/ai/run/${model.id}`,
          model.body(prompt, seed),
          {
            headers: { 'Authorization': `Bearer ${CF_API_TOKEN}`, 'Content-Type': 'application/json' },
            timeout: 90000
            // NO responseType: arraybuffer — CF returns JSON with base64 image
          }
        );
        // CF returns { result: { image: "base64..." } } or { image: "base64..." }
        const b64 = response.data?.result?.image || response.data?.image;
        if (b64 && b64.length > 1000) {
          fs.writeFileSync(outputPath, Buffer.from(b64, 'base64'));
          console.log(`[Image] CF ${model.id.split('/').pop()} ✅`);
          return;
        }
        console.log(`[Image] CF ${model.id.split('/').pop()} — empty response`);
      } catch (err) {
        const status = err.response?.status || 0;
        const msg = err.response?.data?.errors?.[0]?.message || err.message;
        console.log(`[Image] CF ${model.id.split('/').pop()} failed ${status}: ${msg}`);
...
// ROUTES
// ─────────────────────────────────────────────
app.get('/', (req, res) => res.json({ app: 'VidForge AI', version: '10.1.0', status: 'running' }));

app.get('/test-image', async (req, res) => {
  if (!CF_ACCOUNT_ID || !CF_API_TOKEN) return res.json({ success: false, error: 'Cloudflare keys not set' });
  try {
    const testPrompt = req.query.prompt || 'anime cartoon character, cute, vibrant colors, portrait';
    const response = await axios.post(
...
app.listen(PORT, () => {
  console.log(`\n🚀 VidForge AI v10.1 — spawn FFmpeg (no buffer limit) + Cloudflare AI`);
