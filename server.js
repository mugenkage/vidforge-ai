...
//  ✅ 2 CF images/scene (wide + close), 3-account rotation
...
// Wide + close images per scene on alternating accounts
// ─────────────────────────────────────────────
...
// ─────────────────────────────────────────────
// STEP 6 — BUILD IMAGE CLIP (Ken Burns)
// 3 cinematic camera moves per image
// ─────────────────────────────────────────────
async function buildImageClip(imgPath, outputPath, duration, viewIndex, isCloseUp) {
  ...
  const wideViews = [
    `scale=768:1344:force_original_aspect_ratio=decrease,pad=768:1344:(ow-iw)/2:(oh-ih)/2:black,zoompan=z='1+0.04*on/${totalFrames}':x='iw/2-(iw/zoom/2)':y='ih/2-(ih/zoom/2)':d=1:s=768x1344:fps=${fps}`,
    `scale=768:1344:force_original_aspect_ratio=decrease,pad=768:1344:(ow-iw)/2:(oh-ih)/2:black,zoompan=z='1.03':x='iw/2-(iw/zoom/2)':y='ih*0.15-(ih/zoom/2)+ih*0.12*on/${totalFrames}':d=1:s=768x1344:fps=${fps}`,
    `scale=768:1344:force_original_aspect_ratio=decrease,pad=768:1344:(ow-iw)/2:(oh-ih)/2:black,zoompan=z='1.1-0.06*on/${totalFrames}':x='iw/2-(iw/zoom/2)':y='ih*0.05-(ih/zoom/2)':d=1:s=768x1344:fps=${fps}`,
  ];

  const closeViews = [
    `scale=768:1344:force_original_aspect_ratio=decrease,pad=768:1344:(ow-iw)/2:(oh-ih)/2:black,zoompan=z='1.05+0.08*on/${totalFrames}':x='iw/2-(iw/zoom/2)':y='ih*0.02-(ih/zoom/2)':d=1:s=768x1344:fps=${fps}`,
    `scale=768:1344:force_original_aspect_ratio=decrease,pad=768:1344:(ow-iw)/2:(oh-ih)/2:black,zoompan=z='1.08':x='iw*0.52-(iw/zoom/2)':y='ih*0.02-(ih/zoom/2)':d=1:s=768x1344:fps=${fps}`,
    `scale=768:1344:force_original_aspect_ratio=decrease,pad=768:1344:(ow-iw)/2:(oh-ih)/2:black,zoompan=z='1.06+0.02*on/${totalFrames}':x='iw/2-(iw/zoom/2)':y='ih*0.03-(ih/zoom/2)':d=1:s=768x1344:fps=${fps}`,
  ];

  const vf = (isCloseUp ? closeViews : wideViews)[viewIndex % 3];
...
// 2 images × 3 clips = 6 shots per scene
...
async function buildSceneVideo(wideImgPath, closeImgPath, caption, outputPath, duration, style, wordTimestamps, sceneIndex, category, location) {
  ...
  const isSigma  = sceneIndex === 5; // Scene 6 — sigma twist

  // 3 clips: wide → close → wide (sigma scene gets close → wide → close for drama)
  const clipConfigs = isSigma
    ? [
        { img: closeImgPath, view: 0, isClose: true  },
        { img: wideImgPath,  view: 0, isClose: false },
        { img: closeImgPath, view: 1, isClose: true  },
      ]
    : [
        { img: wideImgPath,  view: 0, isClose: false },
        { img: closeImgPath, view: 0, isClose: true  },
  ...
  console.log(`[FFmpeg] Scene ${sceneIndex + 1} ✅${isSigma ? ' 🔱 SIGMA' : ''}`);
}
...
async function runPostingCycle() {
  ...
  try {
    ...
    // 3 — Generate 2 images + voice per scene (6 scenes)
    console.log(`\n[Step 2] Generating images + voice for 6 scenes (lang: ${dubLang.label})...`);
    const sceneData = [];
    ...
    for (let i = 0; i < script.scenes.length; i++) {
      ...
      // Generate wide + close images + voice in parallel
      await Promise.all([
        generateImage(scene.widePrompt,  wideImg,   characterSeed,     i * 2),
        generateImage(scene.closePrompt, closeImg,  characterSeed + 1, i * 2 + 1),
        generateVoice(narration, audioPath, voice.id, category, dubLang.ttsCode)
      ]);

      // Word-level timestamps for caption sync (English only — Whisper works best in EN)
...
app.get('/', (req, res) => res.json({
  app: 'VidForge AI', version: '16.0.0', status: '🚀 Running',
  features: [
    '6 scenes/video', 'Scene 6 = Sigma Twist', 'Caption fix (no overflow)',
    'Phonk BG music', 'Multi-language dubbing', 'Voice rotation (5 voices)',
    'Character consistency', '15 videos/day @ 96-min intervals',
    '2 CF images/scene', 'Word-sync captions', 'Auto thumbnail', 'Pinned comment'
  ]
}));
...
app.post('/generate', async (req, res) => {
  ...
  res.json({ message: `🎬 Generating Ep ${seriesState.currentEpisode} (6 scenes, phonk BG, ${seriesState.dubLang?.label || 'English'})!` });
  runPostingCycle();
});
...
app.listen(PORT, () => {
  console.log(`\n${'═'.repeat(60)}`);
  console.log(`🚀 VidForge AI v16.0 — Sigma Phonk Edition`);
  ...
  console.log(`🎵  Phonk: ${fs.existsSync(PHONK_PATH) ? '✅ phonk.mp3 ready' : '❌ phonk.mp3 MISSING — upload to repo root'}`);
  console.log(`🔱  6 scenes/video | Scene 6 = SIGMA TWIST always`);
  console.log(`📊  15 videos/day @ 96-min intervals`);
  console.log(`☁️   CF Accounts: ${CF_ACCOUNTS.length} | Logo: ${fs.existsSync(LOGO_PATH) ? '✅' : '❌'}`);
  console.log(`${'═'.repeat(60)}\n`);
});
