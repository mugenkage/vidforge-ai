...
// ─────────────────────────────────────────────
// Cloudflare free tier models — only ones that work on free plan
const CF_MODELS = [
  { id: '@cf/black-forest-labs/flux-1-schnell',          body: p => ({ prompt: p, num_steps: 4 }) },
  { id: '@cf/stabilityai/stable-diffusion-xl-base-1.0', body: p => ({ prompt: p }) },
  { id: '@cf/lykon/dreamshaper-8-lcm',                   body: p => ({ prompt: p, num_steps: 8 }) },
  { id: '@cf/bytedance/stable-diffusion-xl-lightning',   body: p => ({ prompt: p, num_steps: 4 }) },
];
let cfModelIndex = 0;
let lastCfCallTime = 0; // track last CF call for rate limit spacing

async function generateCartoonImage(prompt, outputPath, seed) {
  // Try ALL Cloudflare models, rotating to spread rate limits
  if (CF_ACCOUNT_ID && CF_API_TOKEN) {
    for (let attempt = 0; attempt < CF_MODELS.length; attempt++) {
      const model = CF_MODELS[cfModelIndex % CF_MODELS.length];
      cfModelIndex++;
      try {
        // Wait at least 8s between CF calls to avoid 429
        const now = Date.now();
        const timeSinceLast = now - lastCfCallTime;
        if (timeSinceLast < 8000) {
          const waitMs = 8000 - timeSinceLast;
          console.log(`[Image] Waiting ${(waitMs/1000).toFixed(1)}s for CF rate limit...`);
          await new Promise(r => setTimeout(r, waitMs));
        }
        lastCfCallTime = Date.now();
        ...
        console.log(`[Image] CF ${model.id.split('/').pop()} failed: ${status}`);
        await new Promise(r => setTimeout(r, status === 429 ? 500 : 1500));
      }
    }
  }
