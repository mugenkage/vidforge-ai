...
async function generateCartoonImage(prompt, outputPath) {
  ...
  // 1. Pollinations — multiple domain variants
  const pollinationsUrls = [
    `https://pollinations.ai/p/${encoded}?width=576&height=1024&seed=${seed}&nologo=true&model=flux`,
    `https://image.pollinations.ai/prompt/${encoded}?width=576&height=1024&seed=${seed}&nologo=true&model=flux&enhance=true`,
    `https://image.pollinations.ai/prompt/${encoded}?width=576&height=1024&seed=${seed}&nologo=true&model=turbo`,
  ];
  ...
  // 2. Stable Diffusion via Replicate (free tier, no key needed for basic)
  try {
    console.log('[Image] Trying Stable Diffusion XL via Replicate...');
    const response = await axios({
      method: 'GET',
      url: `https://image.pollinations.ai/prompt/${encoded}?width=576&height=1024&seed=${seed}&model=sdxl&nologo=true`,
      responseType: 'arraybuffer', timeout: 60000,
      headers: { 'User-Agent': 'Mozilla/5.0' }
    });
    if (response.data && response.data.length > 10000) {
      fs.writeFileSync(outputPath, Buffer.from(response.data));
      console.log('[Image] SDXL saved!');
      return;
    }
  } catch (err) {
    console.log(`[Image] SDXL failed: ${err.message}`);
  }
  ...
  // 4. Lexica.art (free, no key, anime-friendly)
  try {
    console.log('[Image] Trying Lexica...');
    const searchRes = await axios({
      method: 'GET',
      url: `https://lexica.art/api/v1/search?q=${encoded}`,
      timeout: 15000,
      headers: { 'User-Agent': 'Mozilla/5.0' }
    });
    const images = searchRes.data?.images;
    if (images && images.length > 0) {
      const imgUrl = images[Math.floor(Math.random() * Math.min(5, images.length))].src;
      const imgRes = await axios({ method: 'GET', url: imgUrl, responseType: 'arraybuffer', timeout: 20000 });
      if (imgRes.data && imgRes.data.length > 5000) {
        fs.writeFileSync(outputPath, Buffer.from(imgRes.data));
        console.log('[Image] Lexica saved!');
        return;
      }
    }
  } catch (err) {
    console.log(`[Image] Lexica failed: ${err.message}`);
  }

  // 5. Picsum last resort
  try {
