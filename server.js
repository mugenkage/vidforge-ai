...
// ─────────────────────────────────────────────
// STEP 2 — HUGGING FACE: Real AI Cartoon Image
// ─────────────────────────────────────────────
async function generateCartoonImage(prompt, outputPath) {
  const fullPrompt = prompt + ', vertical composition, 9:16 aspect ratio, no text, no watermark, high quality, detailed';

  // Try HuggingFace models first
  if (HF_API_KEY) {
    for (const model of HF_MODELS) {
      try {
        console.log(`[HF] Trying ${model.split('/')[1]}...`);
        const response = await axios.post(
          `https://api-inference.huggingface.co/models/${model}`,
          {
            inputs: fullPrompt,
            parameters: {
              negative_prompt: 'text, watermark, blurry, low quality, ugly, bad anatomy, landscape, horizontal',
        ...
        if (response.data && response.data.length > 5000) {
          ...
          console.log(`[HF] ✅ Real AI cartoon image saved!`);
          return;
        }
      } catch (err) {
        const msg = err.response?.data ? Buffer.from(err.response.data).toString() : err.message;
        console.log(`[HF] ${model.split('/')[1]} failed: ${msg.substring(0, 100)}`);
  ...
  console.log('[Image] HF failed, trying Pollinations...');
...
app.listen(PORT, () => {
  console.log(`\n🚀 VidForge AI v7.0 — Real AI Cartoon Images`);
  console.log(`🎨 ${ART_STYLES.length} styles | 📚 ${VIDEO_CATEGORIES.length} categories`);
  console.log(`🤗 HuggingFace: ${HF_API_KEY ? 'Connected ✅' : 'Not set ❌'}`);
