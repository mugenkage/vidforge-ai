...
function runFFmpeg(args) {
  return new Promise((resolve, reject) => {
    ...
    let errOut = '';
    ff.stderr.on('data', d => {
      // Only keep last 500 chars to avoid memory buildup
      errOut = (errOut + d.toString()).slice(-500);
    });
    ff.on('close', code => {
      if (code === 0) resolve();
...
function runFFprobe(filePath) {
  return new Promise((resolve, reject) => {
    ...
    ff.on('close', code => {
      if (code === 0) resolve(parseFloat(out.trim()) || 5.0);
      else resolve(5.0); // safe default
    });
    ff.on('error', () => resolve(5.0)); // safe default
  });
}
...
// ─────────────────────────────────────────────
// ART STYLES — quality prompts with masterpiece tag
// ─────────────────────────────────────────────
const ART_STYLES = [
  {
    name: 'Chibi Cartoon',
    prompt: 'masterpiece chibi cartoon style, cute oversized head, tiny body, big sparkly eyes, bold black outlines, flat bright colors, kawaii, adorable expression, high quality illustration'
  },
  {
    name: 'Anime Manga',
    prompt: 'masterpiece anime manga style, big expressive eyes, dynamic hair, detailed cel shading, clean lines, vibrant colors, emotional expression, japanese animation, high quality'
  },
  {
    name: 'Comic Book Hero',
    prompt: 'masterpiece american comic book style, bold ink outlines, dynamic pose, halftone shading, primary colors, superhero aesthetic, dramatic lighting, high quality illustration'
  },
  {
    name: 'Pixar 3D',
    prompt: 'masterpiece pixar 3d animation style, smooth subsurface skin, expressive cartoon face, colorful background, disney quality render, soft lighting, high detail'
  },
  {
    name: 'Retro Cartoon',
    prompt: 'masterpiece retro 1950s cartoon style, rubberhose animation, simple round shapes, limited color palette, bold black outlines, vintage cartoon, high quality'
  },
  {
    name: 'Dark Anime',
    prompt: 'masterpiece dark anime style, dramatic cinematic lighting, glowing eyes, detailed shading, mature manga aesthetic, moody atmosphere, sharp lines, high quality'
  },
  {
    name: 'Storybook',
    prompt: 'masterpiece childrens storybook illustration, watercolor textures, soft pastel colors, whimsical characters, gentle lines, fairy tale aesthetic, high quality'
  },
  {
    name: 'Street Art Cartoon',
    prompt: 'masterpiece graffiti street art cartoon style, bold flat colors, sharp geometric shapes, urban aesthetic, spray paint texture, expressive character, high quality'
  }
];
...
// ─────────────────────────────────────────────
// CAPTION CLEANER — strips ALL non-ASCII safely
// ─────────────────────────────────────────────
...
STRICT RULES:
1. Design ONE main character used in ALL 4 scenes (same species, colors, outfit - only emotion changes)
2. Scene captions: MAX 5 WORDS, ALL CAPS, ENGLISH LETTERS AND SPACES ONLY - NO emojis NO symbols NO punctuation
3. Make story emotional: Hook -> Buildup -> Climax -> Surprising Twist

Respond with ONLY valid JSON (no markdown, no backticks, no explanation):
{
  "title": "Catchy title under 55 chars",
  "series": "Short series name",
  "description": "2 engaging sentences with 2 emojis",
  "hashtags": "#cartoon #shorts #viral #fyp #animation #${category.replace(/ /g,'')} #trending #anime #story #episode",
  "character_design": "Detailed description: species, body type, color palette, outfit, hair or ears, key features",
  "scenes": [
    { "narration": "Hook - 1 to 2 engaging sentences", "caption": "FIVE WORD CAPTION", "emotion": "Detailed: emotion, pose, action, background setting" },
    { "narration": "Buildup - 1 to 2 sentences", "caption": "FIVE WORD CAPTION", "emotion": "Detailed: emotion, pose, action, background setting" },
    { "narration": "Climax - 1 to 2 sentences", "caption": "FIVE WORD CAPTION", "emotion": "Detailed: emotion, pose, action, background setting" },
    { "narration": "Twist ending - 1 to 2 sentences", "caption": "FIVE WORD CAPTION", "emotion": "Detailed: emotion, pose, action, background setting" }
  ]
}`;
  ...
  // Build full image prompt for each scene
  script.scenes = script.scenes.map(scene => ({
    ...scene,
    image_prompt: `${artStyle.prompt}, ${script.character_design}, ${scene.emotion}, portrait vertical 9:16, vibrant colors, masterpiece, best quality, no text, no watermark, highly detailed`
  }));
...
// ─────────────────────────────────────────────
// STEP 2 — IMAGE: Cloudflare Workers AI
// Sequential (one at a time) = light on Render
// ─────────────────────────────────────────────
async function generateCartoonImage(prompt, outputPath, seed) {
  // PRIMARY: Cloudflare FLUX
  if (CF_ACCOUNT_ID && CF_API_TOKEN) {
    try {
      ...
      if (res.data && res.data.length > 5000) {
        ...
        console.log('[Image] Cloudflare ✅');
        return;
      }
      console.log('[Image] Cloudflare returned empty response, trying fallback...');
    } catch (err) {
      const code = err.response ? err.response.status : 'timeout';
      console.log(`[Image] Cloudflare error ${code}, trying fallback...`);
    }
  }
  ...
  try {
    const res = await axios({
      method: 'GET',
      url: `https://picsum.photos/seed/${seed}/720/1280`,
      responseType: 'arraybuffer',
      timeout: 20000
    });
    fs.writeFileSync(outputPath, Buffer.from(res.data));
...
// ─────────────────────────────────────────────
// STEP 3 — VOICE: Google TTS (free, reliable)
// ─────────────────────────────────────────────
async function generateVoice(text, outputPath) {
  ...
  const url = `https://translate.google.com/translate_tts?ie=UTF-8&q=${encodeURIComponent(clean)}&tl=en&client=tw-ob&ttsspeed=0.9`;
...
// ─────────────────────────────────────────────
// STEP 4 — FFMPEG: Build one scene video
// 720x1280 quality + ultrafast + caption overlay
// ─────────────────────────────────────────────
async function buildSceneVideo(imgPath, audioPath, caption, outputPath, duration, style) {
  // Clean caption — remove ALL problematic characters
  const clean = cleanCaption(caption);

  // Split into 2 lines if more than 3 words
...
// ─────────────────────────────────────────────
// STEP 5 — FFMPEG: Stitch all scenes + music
// ─────────────────────────────────────────────
async function stitchFinalVideo(sceneVideos, tempDir, outputPath, category) {
  ...
  // Try downloading theme music
  const musicPath = path.join(tempDir, 'music.mp3');
  let hasMusic = false;
  try {
    const musicRes = await axios.get(getThemeMusicUrl(category), {
      responseType: 'arraybuffer',
      timeout: 15000
    });
    ...
    console.log('[Music] Download failed, skipping');
  }
  ...
  if (hasMusic) {
    try {
      ...
      await runFFmpeg([
        ...
        '-i', musicPath,
        '-filter_complex',
        `[1:a]atrim=0:${totalDur.toFixed(2)},asetpts=PTS-STARTPTS,volume=0.10[bg];[0:a]volume=1.0[voice];[voice][bg]amix=inputs=2:duration=first:dropout_transition=2[aout]`,
        '-map', '0:v',
        '-map', '[aout]',
        '-c:v', 'copy',
        '-c:a', 'aac',
      ...
      console.log('[Music] Mixed into video ✅');
      return;
    } catch (e) {
      console.log('[Music] Mix failed, using voice only:', e.message);
    }
  }

  // No music — just copy
  fs.copyFileSync(tempConcat, outputPath);
  console.log('[Video] Final video ready (no music) ✅');
}
...
async function postToInstagram(videoUrl, caption) {
  ...
  const containerId = conRes.data.id;
...
async function runPostingCycle(category = null) {
  ...
  try {
    ...
    // ── STEPS 2+3: Generate image then voice for each scene (sequential = light) ──
    const durations = [];
    for (let i = 0; i < script.scenes.length; i++) {
      ...
      console.log(`[2/5] Scene ${i + 1}/4 — "${cleanCaption(scene.caption)}"`);

      await generateCartoonImage(scene.image_prompt, imgPath, seed + i);
      await generateVoice(scene.narration, audioPath);

      // Get audio duration
      const dur = await runFFprobe(audioPath);
      durations.push(Math.max(dur + 0.8, 3.5));
      console.log(`      Duration: ${durations[i].toFixed(1)}s`);
    }

    // ── STEP 4: Build scene videos ──
    console.log(`\n[3/5] Building scene videos with "${cap.name}" captions...`);
    const sceneVideos = [];
    for (let i = 0; i < script.scenes.length; i++) {
    ...
    console.log('\n[4/5] Stitching final video with theme music...');
    ...
    try {
      ...
      console.error('[5/5] YouTube ❌', err.message);
    ...
    if (results.youtube) {
      ...
      try {
        ...
        console.error('[5/5] Instagram ❌', err.message);
        results.instagramError = err.message;
      }
    }

    console.log(`\n🎉 [VidForge v13] DONE! ${dailyCount}/${DAILY_LIMIT} today`);
    console.log(`🔗 ${results.youtube || 'No YouTube link'}\n`);

  } catch (err) {
    results.error = err.message;
    dailyCount--;
    console.error('\n❌ [VidForge v13] Pipeline failed:', err.message, '\n');
  } finally {
...
app.get('/', (req, res) => res.json({
  ...
  version: '13.0.0',
  status: 'running',
  docs: { generate: 'POST /generate', status: 'GET /status', testImage: 'GET /test-image' }
}));
...
// Test Cloudflare image generation
app.get('/test-image', async (req, res) => {
  if (!CF_ACCOUNT_ID || !CF_API_TOKEN) {
    return res.status(400).json({ error: 'CLOUDFLARE_ACCOUNT_ID or CLOUDFLARE_API_TOKEN not set' });
  }
  try {
    ...
    res.json({ error: 'Empty response from Cloudflare' });
  } catch (err) {
    res.status(500).json({ error: err.response ? `HTTP ${err.response.status}` : err.message });
  }
});
...
app.post('/generate', async (req, res) => {
  if (dailyCount >= DAILY_LIMIT) {
    return res.json({ success: false, message: `Daily limit reached (${DAILY_LIMIT}/day)` });
  }
  const category = (req.body || {}).category || null;
  res.json({ success: true, message: '🎬 Video generation started!', category: category || 'auto' });
...
app.post('/test-script', async (req, res) => {
  ...
  try {
    ...
    res.json({ success: true, category: cat, artStyle: style.name, script });
  } catch (err) {
...
app.get('/auth/youtube/callback', async (req, res) => {
  try {
    ...
    res.json({ message: 'Copy this refresh_token to Render environment!', refresh_token: tokens.refresh_token });
  } catch (err) {
...
app.listen(PORT, () => {
  console.log(`\n🚀 VidForge AI v13.0 — Quality + Stable + Light`);
  console.log(`☁️  Cloudflare AI : ${(CF_ACCOUNT_ID && CF_API_TOKEN) ? 'ENABLED ✅' : 'NOT SET ❌'}`);
