...
// ─────────────────────────────────────────────
// ART STYLES — per category
// ─────────────────────────────────────────────
const CATEGORY_ART = {
  'horror':          { name: 'Dark Anime',      prompt: 'dramatic dark anime style, intense glowing red eyes, detailed shading, horror atmosphere, sharp lines, expressive character, cinematic' },
  'thriller':        { name: 'Dramatic Anime',  prompt: 'dramatic cinematic anime style, intense focused expression, suspenseful atmosphere, sharp clean lines, powerful character, dynamic lighting' },
  'mystery':         { name: 'Noir Anime',      prompt: 'cinematic noir anime style, moody dramatic shadows, muted palette with vivid accent color, mysterious atmosphere, detailed linework' },
  'adventure':       { name: 'Epic Anime',      prompt: 'vibrant epic anime style, large expressive eyes, dynamic action pose, bright saturated colors, energy lines, detailed shading' },
  'superhero':       { name: 'Comic Hero',      prompt: 'american comic book style, bold ink outlines, dynamic powerful pose, halftone shading, primary bold colors, heroic, dramatic lighting' },
  'motivation':      { name: 'Power Anime',     prompt: 'inspirational anime style, determined fierce expression, glowing golden aura, dynamic powerful pose, vibrant warm colors, motivational energy' },
...
// Every 4 hours IST (UTC+5:30):
// 12AM, 4AM, 8AM, 12PM, 4PM, 8PM IST
// = 6:30PM, 10:30PM, 2:30AM, 6:30AM, 10:30AM, 2:30PM UTC
cron.schedule('30 18 * * *', () => { if (dailyCount < DAILY_LIMIT) { console.log('[Cron] 12AM IST'); runPostingCycle(); } });
cron.schedule('30 22 * * *', () => { if (dailyCount < DAILY_LIMIT) { console.log('[Cron] 4AM IST');  runPostingCycle(); } });
cron.schedule('30 2 * * *',  () => { if (dailyCount < DAILY_LIMIT) { console.log('[Cron] 8AM IST');  runPostingCycle(); } });
cron.schedule('30 6 * * *',  () => { if (dailyCount < DAILY_LIMIT) { console.log('[Cron] 12PM IST'); runPostingCycle(); } });
cron.schedule('30 10 * * *', () => { if (dailyCount < DAILY_LIMIT) { console.log('[Cron] 4PM IST');  runPostingCycle(); } });
cron.schedule('30 14 * * *', () => { if (dailyCount < DAILY_LIMIT) { console.log('[Cron] 8PM IST');  runPostingCycle(); } });
...
Return ONLY valid JSON:
{
  ...
  "character_design": "${isFirstEp ? 'VERY DETAILED: species, exact colors, hairstyle, eye color, outfit details, unique features, body type' : characterDesign}",
  ...
  script.scenes = script.scenes.map((scene, i) => ({
    ...scene,
    // Wide shot prompt — full body, character consistent
    widePrompt: `${artStyle.prompt}, ${charDesc}, SAME CHARACTER ALWAYS, ${scene.wide_shot}, ${scene.emotion}, full body shot, detailed background, ${scene.location}, vertical 9:16 portrait, high quality, no text, no watermark`,
    // Close up prompt — face/chest, character consistent
    closePrompt: `${artStyle.prompt}, ${charDesc}, SAME CHARACTER ALWAYS, ${scene.close_shot}, ${scene.emotion}, close up portrait, face and chest, dramatic lighting, intense expression, ${scene.location}, vertical 9:16, high quality, no text, no watermark`,
  }));
...
app.get('/status', (req, res) => res.json({
  ...
  postingTimes: 'IST every 4 hours (12AM, 4AM, 8AM, 12PM, 4PM, 8PM)',
  series: {
...
app.get('/test-image', async (req, res) => {
  ...
  try {
    ...
    const prompt = req.query.prompt || 'dramatic dark anime hero, glowing red eyes, epic pose, 9:16 portrait';
...
app.listen(PORT, () => {
  ...
  console.log(`🕐  Post times: IST 12AM, 4AM, 8AM, 12PM, 4PM, 8PM (every 4 hours)`);
