...
// Multiple Groq accounts — rotate to avoid 429 rate limits
// Add GROQ_API_KEY_2, GROQ_API_KEY_3 etc in Render env vars
const GROQ_KEYS = [
...
// Free models — rotate between them for higher combined limits
const GROQ_MODELS = [
...
// ─────────────────────────────────────────────
const DUBS = [
  { lang: 'en', label: 'English',    ttsCode: 'en', voiceId: 'TxGEqnHWrfWFTfGW9XjX' },
  { lang: 'hi', label: 'Hindi',      ttsCode: 'hi', voiceId: '21m00Tcm4TlvDq8ikWAM' },
  { lang: 'es', label: 'Spanish',    ttsCode: 'es', voiceId: 'VR6AewLTigWG4xSOukaG' },
  { lang: 'pt', label: 'Portuguese', ttsCode: 'pt', voiceId: 'EXAVITQu4vr4xnSDxMaL' },
  { lang: 'fr', label: 'French',     ttsCode: 'fr', voiceId: 'yoZ06aMxZJJ28mfd3POQ' },
  { lang: 'ja', label: 'Japanese',   ttsCode: 'ja', voiceId: null }, // Google TTS only
];
...
const SERIES_CATEGORIES = [
  ...
  'fantasy', 'science fiction', 'superhero', 'motivation', 'comedy', 'samurai'
];
...
const CATEGORY_ART = {
  ...
  'samurai':         { name: 'Samurai Stick',    prompt: 'minimalist stick figure illustration, black ink on white background, japanese samurai stick figure character, wearing simple kimono robe outline, katana sword prop, topknot hair on round head, traditional japanese scene, cherry blossom petals, torii gate or pagoda silhouette in background, bold brushstroke style, sumi-e ink aesthetic, high contrast black and white, no color, no text' },
};
...
const CAPTION_STYLES = {
  ...
  'samurai':         { fontfile: FONT_BOLD,    fontsize: 54, fontcolor: '0xFFFFFF', highlight: '0xFF4400', borderw: 6,  bordercolor: 'black',       box: false },
...
const COLOR_GRADE = {
  ...
  'samurai':         "curves=r='0/0 0.5/0.48 1/0.92':g='0/0 0.5/0.45 1/0.88':b='0/0.02 0.5/0.42 1/0.82',vignette=PI/4",
...
Return ONLY valid JSON (no markdown, no backticks):
{
  ...
  // Retry up to 3 times on 429 — rotate key+model each attempt
  let res, attempt = 0;
  while (attempt < 3) {
    const { key, model } = getNextGroq();
    try {
      res = await axios.post(
        'https://api.groq.com/openai/v1/chat/completions',
        { model, messages: [{ role: 'user', content: prompt }], max_tokens: 2800, temperature: 0.92 },
      ...
      if (err.response?.status === 429) {
        ...
        console.log(`[Groq] 429 — rotating key/model, waiting ${wait/1000}s (attempt ${attempt}/3)...`);
        await new Promise(r => setTimeout(r, wait));
      } else throw err;
    }
  }
  if (!res) throw new Error('Groq script generation failed after 3 retries');
...
// ─────────────────────────────────────────────
// STEP 6 — BUILD SCENE VIDEO
// 1 image, static, full duration — no clips, no concat
// ─────────────────────────────────────────────
async function buildSceneVideo(imgPath, caption, outputPath, duration, style, wordTimestamps, sceneIndex, category, location) {
  ...
  const vf = `scale=768:1344:force_original_aspect_ratio=increase,crop=768:1344,fps=24,fade=t=in:st=0:d=0.3,fade=t=out:st=${fadeOut}:d=0.3,${colorGrade}${rainFilter}${captionPart},${progressFilter}${locationPart}${sigmaBadge}`;

  await runFFmpeg(['-loop', '1', '-i', imgPath, '-c:v', 'libx264', '-preset', 'ultrafast', '-pix_fmt', 'yuv420p', '-t', String(duration), '-vf', vf, '-an', '-y', outputPath]);
...
async function runPostingCycle() {
  ...
  try {
    ...
    for (let i = 0; i < script.scenes.length; i++) {
      ...
      // Samurai: translate to Japanese for voice, keep English for captions
      const isSamurai = category === 'samurai';
      const voiceLang = isSamurai ? 'ja' : dubLang.ttsCode;
      const narration = isSamurai
        ? await translateNarration(scene.narration, 'ja')
        : dubLang.lang !== 'en'
          ? await translateNarration(scene.narration, dubLang.lang)
          : scene.narration;

      // English narration for captions (always)
      const englishNarration = scene.narration;

      // Generate 1 image + voice in parallel
      const [, voiceEngine] = await Promise.all([
        generateImage(scene.widePrompt, imgPath, characterSeed, i),
        generateVoice(narration, audioPath, voice.id, category, voiceLang)
      ]);
      ...
      // Captions always use English narration split evenly (Whisper only on ElevenLabs English)
      let timestamps = null;
      if (voiceEngine === 'elevenlabs' && !isSamurai) {
        timestamps = await getWordTimestamps(audioPath);
      } else {
        const words = englishNarration.replace(/['"]/g, '').trim().split(/\s+/);
        const interval = (duration - 0.6) / words.length;
        timestamps = words.map((word, idx) => ({
          word: word.toUpperCase(),
          start: 0.3 + idx * interval,
          end: 0.3 + (idx + 1) * interval
        }));
        console.log(`[Captions] English word-sync (${words.length} words)`);
      }
