...
async function buildImageClip(imgPath, outputPath, duration, viewIndex) {
  ...
  const views = [
    // Variation 0: slow zoom-in from center (crop shrinks toward center over time)
    `scale=1152:2016:force_original_aspect_ratio=increase,crop=1152:2016,` +
    `crop=w=trunc((1152-192*on/${N})/2)*2:h=trunc((2016-336*on/${N})/2)*2:x=trunc(96*on/${N}/2)*2:y=trunc(168*on/${N}/2)*2,` +
    `scale=768:1344,fps=${fps}`,

    // Variation 1: pan upward slowly (crop window moves up)
    `scale=1152:2016:force_original_aspect_ratio=increase,crop=1152:2016,` +
    `crop=w=768:h=1344:x=192:y=trunc((672-672*on/${N})/2)*2,` +
    `fps=${fps}`,

    // Variation 2: pan downward slowly (crop window moves down)
    `scale=1152:2016:force_original_aspect_ratio=increase,crop=1152:2016,` +
    `crop=w=768:h=1344:x=192:y=trunc(672*on/${N}/2)*2,` +
    `fps=${fps}`,
  ];
    ...
    try {
      await runFFmpeg([
        ...
        '-filter_complex',
          ...
          `[2:a]volume=0.12,atrim=end=${durNum}[phonk];` +
